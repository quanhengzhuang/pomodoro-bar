...
右上角的引号按钮打开“今日指引”：页面显示日期，可直接修改并保存当天的多行内容，也可上下滑动连续浏览最近 30 个自然日内内容非空的历史指引。编辑和展示使用相同的暖黄色斜体、字号、行距、边距与换行宽度。首次使用时，在系统文件选择器中进入 `iCloud Drive/PomodoroBar/` 并选择 `daily-guidance.json`；保存只更新当天内容并保留其他日期。iOS 计时记录保存在 App 自己的沙盒中，不覆盖或删除 Mac 端数据。
